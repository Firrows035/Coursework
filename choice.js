...
var choiceCount=0;
function addChoice(name,buff,description,weight,isDisposable){
    choiceCount++;
    choice[choiceCount]={
        name:name,
        buff:buff,
...
addChoice(`atk+`,
    ...
    `物理攻击力+100%,hp上限-80%, 攻击距离+3`,1,true);
addChoice(`def+`,
    ()=>{
        player.baseDef+=5;
        updatePlayerStat();
...
"技能冷却-50%，mp上限+100%，造成伤害-20%",3,true);
addChoice("atk++/Eatk+",()=>{
    boost.player.atk+=40;
    boost.enemy.atk+=20;
    updatePlayerStat();
},
"物理攻击力+40%，敌人攻击力+20%",4,false);



function randChoice(){
    let c1=random(1,choiceCount);
    if(random(1,choice[c1].weight)>1) return randChoice();
    else return c1;
}
